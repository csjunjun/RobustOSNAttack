...
class decompress_jpeg(nn.Module):
    ...
    def forward(self, y, cb, cr, factor=1,height=256, width=256):
        self.height = height
        self.width = width
...
class block_splitting(nn.Module):
    ...
    def forward(self, image):
        ...
        #modified by jun 20210524
        # image_padded = image
        # right_pad = self.k- width % self.k
        # bottom_pad = self.k- height % self.k
        # if right_pad > 0 or bottom_pad > 0:
        #     image_padded = torch.unsqueeze(image,0)
        #     pad1 = nn.ReplicationPad2d((0,right_pad,0,bottom_pad))
        #     image_padded = pad1(image_padded)[0]
        #     height, width = image_padded.shape[1:3]

        # image_reshaped = image_padded.view(batch_size, height // self.k, self.k, -1, self.k)
        #end
...
class DiffJPEG(nn.Module):
    ...
    def forward(self, x):
        ...
        recovered = self.decompress(y, cb, cr,factor,height, width)
        return recovered
